...
(vendor 自 mathlib4 @274ed6d67d, 两处本地补丁:
 1) 移除悬空引用 nonterminal_simp_check (该 linter 已改写为 Lean);
 2) enumerate → list(enumerate) (上游 WIP 破坏).)
...
def lint(path, fix=False):
    ...
    with path.open(encoding="utf-8", newline="") as f:
        ...
        # 注: nonterminal_simp_check 已改写成 Lean (本 commit Python 侧悬空引用), 移除
